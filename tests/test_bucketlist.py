...
class TestBucketListCrud(unittest.TestCase):
    ...
    def test_returns_false_if_name_is_empty(self):
        self.account_manager.signup("jane", "doe")
        user_id = self.account_manager.get_user_id("jane")
        result = self.bucketlist.create_bucket_list(user_id, "","try thrilling experiences")
    ...
    def test_throws_exception_if_id_is_not_an_integer(self):
        with self.assertRaises(TypeError):
            self.bucketlist.create_bucket_list([], "dare devil","try thrilling experiences")

    def test_updates_bucket_list_name(self):
        self.account_manager.signup("jane", "doe")
        user_id = self.account_manager.get_user_id("jane")
        blist = self.bucketlist.create_bucket_list(user_id, "dare devil","try thrilling experiences")
        bucketlist_id = self.bucketlist.get_bucketlist_id(user_id, "dare devil")
        result = self.bucketlist.update_bucket_list(bucketlist_id, "graduate",
                                                    "get a degreee")
    ...
    def test_updates_bucket_list_description(self):
        ...
        blist = self.bucketlist.create_bucket_list(user_id, "dare devil","try thrilling experiences")
        bucketlist_id = self.bucketlist.get_bucketlist_id(user_id, "dare devil")
        result = self.bucketlist.update_bucket_list(bucketlist_id,"dare devil",
                                                    "try something new")
        self.assertEqual(result, True)
