...
class TestBucketListItems(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.bucketlist.create_bucket_list(user_id,
                                            "dare devil",
                                            "try out experiences")
        self.bucketlist_id = self.bucketlist.get_bucketlist_id(user_id,
                                                               "dare devil")
    ...
    def test_creates_new_item(self):
        item = self.bucketlist_items.create_item(self.bucketlist_id,
                                                 "go bungee jumping")
    ...
    def test_duplicate_items(self):
        item = self.bucketlist_items.create_item(self.bucketlist_id,
                                                 "go bungee jumping")
        item2 = self.bucketlist_items.create_item(self.bucketlist_id,
                                                  "go bungee jumping")
    ...
    def test_updates_item(self):
        ...
        update = self.bucketlist_items.update_item(item_id,
                                                   "go climbing",
                                                   "not done")
    ...
    def test_doesnt_update_nonexisting_item(self):
        item = self.bucketlist_items.create_item(self.bucketlist_id,
                                                 "go bungee jumping")
        update = self.bucketlist_items.update_item(5, "go climbing",
                                                   "not done")
        self.assertFalse(update)

    def test_get_item_by_bucketlist_id(self):
        blist_id = self.bucketlist_id
        self.bucketlist_items.create_item(blist_id, "go bungee jumping")
        items = self.bucketlist_items.get_item_by_bucketlist_id(blist_id)
